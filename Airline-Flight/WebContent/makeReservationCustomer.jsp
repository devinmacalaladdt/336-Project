...
<% 
	...
	if(!(rs.next())){
		...
		if(!(rs1.next())){
			...
			out.println("There are no Resevervations available, would you like to join waitlist?");
			out.println("<form action='addWaitlistCustomer.jsp'><select name='waitlist'><option value='"+request.getParameter("accountname")+","+flight+"'>Add</option><option value='dont'>Don't Add</option></select><input type='submit' value='submit'/></form>");
